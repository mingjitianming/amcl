...
main(int argc, char** argv)
{
  ...
  signal(SIGINT, sigintHandler);  //中断信号 如:ctrl-C
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end())
  {
    ...
    tf::Stamped<tf::Pose> laser_pose;  //base->laser
    try
    {
      this->tf_->transformPose(base_frame_id_, ident, laser_pose);  //输出laser_poes
    }
    catch(tf::TransformException& e)
    {
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  tf::StampedTransform tx_odom;  //两帧间的相对变换
  try
  {
    ros::Time now = ros::Time::now();
